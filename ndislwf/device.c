...
#pragma NDIS_INIT_FUNCTION(ndislwfRegisterDevice)

#define IOCTL_CHANGE_BLOCK_LIST 0

block_list_t* headRecive = NULL;
block_list_t* headSend = NULL;


_IRQL_requires_max_(PASSIVE_LEVEL)
NDIS_STATUS
ndislwfRegisterDevice(
    VOID
    )
{
    ...
    DEBUGP(DL_TRACE, "==>ndislwfRegisterDevice\n");
   
    
    NdisZeroMemory(DispatchTable, (IRP_MJ_MAXIMUM_FUNCTION+1) * sizeof(PDRIVER_DISPATCH));
    
    DispatchTable[IRP_MJ_CREATE] = ndislwfDispatch;
    DispatchTable[IRP_MJ_CLEANUP] = ndislwfDispatch;
    DispatchTable[IRP_MJ_CLOSE] = ndislwfDispatch;
    DispatchTable[IRP_MJ_DEVICE_CONTROL] = ndislwfDeviceIoControl;
    ...
    DEBUGP(DL_TRACE, "<==ndislwfRegisterDevice: %x\n", Status);
...
block_list_t* addIPinBlock(block_list_t* head, UINT32 IP)
{
    block_list_t* newIP = NdisAllocateMemoryWithTagPriority(FilterDriverHandle, sizeof(block_list_t), FILTER_ALLOC_TAG, LowPoolPriority);
...
block_list_t* deleteIPfromBlock(block_list_t* head, UINT32 IP)
{ 
...
void removeList(block_list_t* head)
{
    block_list_t* current;
    while (head)
    {
        current = head;
        head = head->next;
    ...
    )
{
    ...
    switch (IrpSp->Parameters.DeviceIoControl.IoControlCode)
    {
        ...
        case IOCTL_FILTER_RESTART_ALL:
            break;
        case IOCTL_CHANGE_BLOCK_LIST:
        {
           
            CHANGE_LIST_IP_INFO* info = (CHANGE_LIST_IP_INFO*)Irp->AssociatedIrp.SystemBuffer;
            switch (info->type)
            {
                case ADD_RECEIVE_IP:
                    headRecive = addIPinBlock(headRecive, info->IP);
                    break;
                case DELETE_RECEIVE_IP:
                    headRecive = deleteIPfromBlock(headRecive, info->IP);
                    break;
                case CLEAR_RECEIVE_IP:
                    removeList(headRecive);
                    headRecive = NULL;
                    break;
                case ADD_SEND_IP:
                    headSend = addIPinBlock(headSend, info->IP);
                    break;
                case DELETE_SEND_IP:
                    headSend = deleteIPfromBlock(headSend, info->IP);
                    break;
                case CLEAR_SEND_IP:
                    removeList(headSend);
                    headSend = NULL;
