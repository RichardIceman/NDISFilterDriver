...
BOOLEAN             isDrop(PNET_BUFFER_LIST pCurrentNetBufferList, block_list_t* head, BOOLEAN isReceive);
...
--*/
{
    ...
    do
    {
        ...
        PNET_BUFFER_LIST pKeepNetBufferList = NULL;
        PNET_BUFFER_LIST pKeepTail = NULL;
        PNET_BUFFER_LIST pDropNetBufferList = NULL;
        ...
        {
            //собираем два списка 
            //1 с пакетами, которые дропаем
            //2 с остальными
            pNext = NET_BUFFER_LIST_NEXT_NBL(CurrNbl);

            if (isDrop(CurrNbl, headSend, FALSE))
            {
                NET_BUFFER_LIST_STATUS(CurrNbl) = NDIS_STATUS_FAILURE;
                if (pDropNetBufferList == NULL)
                {
                    pDropNetBufferList = CurrNbl;
                    pDropTail = CurrNbl;
                    pDropTail->Next = NULL;
                }
                else
                {
                    pDropTail->Next = CurrNbl;
                    pDropTail = CurrNbl;
                    pDropTail->Next = NULL;
                }
            }
            ...
            {
                if (pKeepNetBufferList == NULL)
                {
                    pKeepNetBufferList = CurrNbl;
                    pKeepTail = CurrNbl;
                    pKeepTail->Next = NULL;
                }
                else
                {
                    pKeepTail->Next = CurrNbl;
                    pKeepTail = CurrNbl;
                    pKeepTail->Next = NULL;
                }
            }
        ...
        if (pKeepNetBufferList)
            NdisFSendNetBufferLists(pFilter->FilterHandle, pKeepNetBufferList, PortNumber, SendFlags);
...
--*/
{
    ...
    do
    {
        ...
        {        
            ...
            PNET_BUFFER_LIST pNext;

            if NDIS_TEST_RECEIVE_CANNOT_PEND(ReceiveFlags)
            {
                //помечаем которые, нужно принять
                while (pCurrentNetBufferList)
                {
                    pNext = NET_BUFFER_LIST_NEXT_NBL(pCurrentNetBufferList);
                    if (!isDrop(pCurrentNetBufferList, headRecive, TRUE))
                    {                 
                        pCurrentNetBufferList->Next = NULL;
                        ...
                        pCurrentNetBufferList->Next = pNext;
                    }
            ...
            else
            {
                ...
                while (pCurrentNetBufferList)
                {
                    ...
                    if (isDrop(pCurrentNetBufferList, headRecive, TRUE))
                    {
                        ...
                        if (pDropNetBufferList == NULL)
                        {
                            pDropNetBufferList = pCurrentNetBufferList;
                            pDropTail = pCurrentNetBufferList;
                            pDropTail->Next = NULL;
                        }
                        else
                        {
                            pDropTail->Next = pCurrentNetBufferList;
                            pDropTail = pCurrentNetBufferList;
                            pDropTail->Next = NULL;
                        }
                    }
                    else
                    {
                        if (pKeepNetBufferList == NULL)
                        {
                            pKeepNetBufferList = pCurrentNetBufferList;
                            pKeepTail = pCurrentNetBufferList;
                            pKeepTail->Next = NULL;
                        }
                        else
                        {
                            pKeepTail->Next = pCurrentNetBufferList;
                            pKeepTail = pCurrentNetBufferList;
                            pKeepTail->Next = NULL;
                        }
                    }
...
BOOLEAN isDrop(PNET_BUFFER_LIST pCurrentNetBufferList, block_list_t* head, BOOLEAN isReceive)
{
    ...
    //достаем MDL
    char* buffer = (char*)NdisGetDataBuffer(
        pCurrentNetBuffer,
        netbuffer_size,
        NULL,
        1,
    ...
    //парсим шапку Ethernet
    eth_header_t* pEthHeader = (eth_header_t*)buffer;

    if (!pEthHeader) return FALSE;

    if (pEthHeader->ether_type == RtlUshortByteSwap(NDIS_ETH_TYPE_IPV4))
    {
        //парсим шапку IPv4
        ipv4_header_t* pIpHeader = (ipv4_header_t*)(buffer + sizeof(eth_header_t));

        UINT32 ip = isReceive ? pIpHeader->src_addr : pIpHeader->dest_addr;


        if (pIpHeader->verAndHlen >> 4 == IPPROTO_IPV4)
        {
            block_list_t* current = head;
            while (current)
            {
                //подсеть или IP заблокированы
                if ((current->ip & ip) == current->ip)
                {
                    return TRUE;
                }
